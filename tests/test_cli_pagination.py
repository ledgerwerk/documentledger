...
def test_sources_list_filters_and_omits_hashes_by_default(project, runner) -> None:
    ...
    data = invoke_json(runner, ["sources", "list", "--kind", "function", "--path-prefix", "documentledger"])["result"]["sources"]
    assert data
...
def test_sources_list_ids_only_and_cursor(project, runner) -> None:
    ...
    write_precision_sample(project)
    invoke_json(runner, ["scan"])
    first = invoke_json(runner, ["sources", "list", "--ids-only", "--limit", "1"])["result"]
    second = invoke_json(runner, ["sources", "list", "--ids-only", "--limit", "1", "--cursor", str(first["next_cursor"])])["result"]
...
def test_docs_sections_outline_is_compact(project, runner) -> None:
    invoke_json(runner, ["init"])
    write_precision_sample(project)
    data = invoke_json(runner, ["docs", "sections", "--doc", "docs/usage.md", "--outline"])["result"]["docs"][0]["sections"]
    assert data
...
def test_invalid_cursor_returns_structured_error(project, runner) -> None:
    invoke_json(runner, ["init"])
    result = runner.invoke(app, ["--json", "sources", "list", "--cursor", "oops"])
    assert result.exit_code != 0
    assert "invalid_cursor" in result.output
