...
def test_docs_sections_and_affected_show_precise_section(project: Path, runner: CliRunner) -> None:
    ...
    sections = invoke_json(runner, ["docs", "sections", "--doc", "docs/usage.md"])["result"]["docs"][0]["sections"]
    ...
    invoke_json(runner, ["scan"])
    affected = invoke_json(runner, ["docs", "affected"])["result"]["affected_sections"]
...
def test_mark_fresh_section_clears_live_affectedness_without_new_scan(project: Path, runner: CliRunner) -> None:
    ...
    invoke_json(runner, ["scan"])
    before = invoke_json(runner, ["docs", "affected"])["result"]["affected_sections"]
    assert before
    update = invoke_json(
        runner,
        [
    ...
    after = invoke_json(runner, ["docs", "affected"])["result"]["affected_sections"]
